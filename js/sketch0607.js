...
function game(){
	...
	for (var i=0; i<SLOT_NUMBER; i++){
		...
		for (var j=0; j<SLOT_LENGTH; j++){
			//index = Math.floor(Math.random()*deck.cards.length);
...
function rangePalette(){
	var flag = false;
	var d  = mouse.distanceTo(mark);
	if (d<0.1)
		flag = true;
	//console.log(d)
	return flag;
...
function addPalette(){
	...
	deck.palettes[0].push( dragIndex);
	deck.hands[0].splice(dragIndex, 1);
	deck.cards[dragIndex].mesh.position.set(
		deck.offsets[0].x/2+deck.palettes[0].length*deck.cards[dragIndex].size.x,
		deck.offsets[0].y/2,
		deck.offsets[0].z/2
	);
	console.log(deck.palettes);
	//deck.cards[dragIndex]
}
function spin(n){
	if (ticks<n && !lastMove){
		//console.log(ticks, n);
...
function getScore(){
	for (var i=0; i<slotMeshs.length; i++){
		...
		if (ticks==stopCount+2)
			console.log(A,	slotValues[i][victoryIndexs[i].index].name);
	}
